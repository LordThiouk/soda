...
class ChannelService {
  ...
  async importRadioStations(overwrite = false) {
    try {
      ...
      const response = await axios.get('https://de1.api.radio-browser.info/json/stations/bycountry/senegal');
      ...
      logger.info(`${stations.length} stations trouvées sur RadioBrowser`);
      ...
      for (const station of stations) {
        try {
          ...
          const stationData = {
            ...
            country: 'Sénégal',
            language: station.language || 'Français',
            status: 'active'
          };
